...
nautobot_device()
if 'host' in nautobot_device.device[0]['name']:
    for iface in nautobot_device.device[0]['interfaces']:
        ...
        if 'Ethernet' in iface['name']:
            if iface['label'] == 'trunk':
                ...
                try:
                    ...
                    separator = ","
                    print(" switchport trunk allowed vlan add %s" % (separator.join(map(str, vlan_list))))
                except Exception:
                    pass
...
elif 'leaf' in nautobot_device.device[0]['name']:
    for iface in nautobot_device.device[0]['interfaces']:
        # Ethernet Interface Configuration
        if 'Ethernet' in iface['name']:
            if iface['label'] == 'trunk':
