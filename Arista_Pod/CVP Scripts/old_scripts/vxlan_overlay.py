...
def conf_vrf_vlans_svi():
    ...
    for item in device_name:
        if item.startswith('hostname'):
            ...
            payload = json.dumps({
            "query": "query ($device: [String]) { devices(name__isw: $device){name config_context local_asn: cf_device_bgp viritual_router_mac: cf_virtual_router_mac tags {slug} site {vlans {name tenant {name} vid vxlan_rt: cf_vxlan_vlan_rt role {slug}}} interfaces {name role: cf_role virtual_router: cf_virtual_router_ipv4 vlan_vni: cf_vxlan_vlan_vni label description enabled mode lag {name} ip_addresses {address vrf {name rd}} connected_interface{device{name}name ip_addresses{address}}}}}",
            "variables": {
            "device": 'leaf1-dc1'
            }
            })
...
def conf_vxlan1():
    url = "http://192.168.130.109:8000/api/graphql/"
    payload = json.dumps({
    "query": "query {vrfs {name rd tenant{name}}}"
    })
    headers = {
    'Content-Type': 'application/json',
    'Authorization': 'Token c7fdc6be609a244bb1e851c5e47b3ccd9d990b58',
...
if 'leaf' in device[0]['name']:
    ...
    if conf_vxlan1.vrfs:
        for vrf in conf_vxlan1.vrfs:
            for tenant in device[0]['config_context']['vrfs']:
                try:
                    if vrf['tenant']['name'] == tenant['name']:
                        ...
                        for iface in device[0]['interfaces']:
                            if iface['name'] == 'Loopback1':
                                for ip in iface['ip_addresses']:
                                    ...
                                    print("    route-target import evpn %s:%s" % (vrf['rd'], vrf['rd']))
                                    print("    route-target export evpn %s:%s" % (vrf['rd'], vrf['rd']))
                except Exception:
                    pass
